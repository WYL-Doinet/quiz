<template>
    <DashboardLayout>
        <form class="w-lg space-y-5" @submit.prevent="submit">
            <div>
                <label for="name"
                    :class="['block mb-2 text-sm font-medium  dark:text-white', form.errors.name ? 'text-red-600' : 'text-gray-900']">
                    Category Name</label>
                <div class="relative">
                    <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none">
                        <svg :class="['w-4 h-4  dark:text-gray-400', form.errors.name ? 'text-red-600' : 'text-gray-500']"
